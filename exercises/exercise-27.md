...
AI需要予測システムにより、POS/気象/イベントデータを統合してSageMakerモデルで「商品×店舗×日」の需要予測を行い、自動発注推奨を実現
...
architecture-beta
    ...
    service pos(database)[POS Data] in data_layer
    service weather(server)[Weather API] in data_layer
    service event(disk)[Event Data] in data_layer
    service datalake(logos:aws-s3)[S3 Data Lake] in data_layer

    group ml_layer(server)[ML Layer] in aws
    service studio(server)[SageMaker Studio] in ml_layer
    service processing(server)[Processing Job] in ml_layer
    service training(server)[Training Job] in ml_layer
    service registry(database)[Model Registry] in ml_layer

    group inference_layer(server)[Inference Layer] in aws
    service batch(server)[Batch Transform] in inference_layer
    service endpoint(server)[Realtime Endpoint] in inference_layer
    service s3results(logos:aws-s3)[S3 Results] in inference_layer
    service apigw(logos:aws-api-gateway)[API Gateway] in inference_layer
...
| コンポーネント | 役割 |
|----------------|------|
| **POS Data** | POSトランザクションデータ（RDS/S3） |
| **Weather API** | 気象データ（API→S3） |
| **Event Data** | イベントデータ（手動→S3） |
| **S3 Data Lake** | データレイク（raw/processed/features） |
| **SageMaker Studio** | Jupyter Notebookでの開発環境 |
| **Processing Job** | データ前処理ジョブ |
| **Training Job** | モデル学習ジョブ |
| **Model Registry** | モデルバージョン管理 |
| **Batch Transform** | 日次バッチ予測 |
| **Realtime Endpoint** | リアルタイム推論エンドポイント |
| **S3 Results** | 予測結果の保存 |
| **API Gateway** | 予測API公開 |
...
**状況:**
本番稼働後3ヶ月で、予測精度が徐々に低下している。MAPEが当初15%だったが、現在は25%まで悪化。
...
**状況:**
リアルタイム推論エンドポイントのレイテンシがSLO（500ms）を超えるケースが増加している。
...
**課題:**
商品カテゴリごとに異なるモデルを使い分けるマルチモデル戦略を設計してください。
...
**課題:**
新しいモデルバージョンを安全にデプロイするためのA/Bテスト基盤を設計してください。
...
**シナリオ:**
特徴量の管理と再利用を効率化するため、SageMaker Feature Storeを導入したい。
...
**次の課題**: [課題28](exercise-28.md)

**前の課題**: [課題26](exercise-26.md)
