...
```mermaid
architecture-beta
    group medconnect(cloud)[MedConnect Auth Platform]

    group clients(server)[Client Applications] in medconnect
    group cognito(logos:aws-cognito)[Amazon Cognito] in medconnect
    group api(logos:aws-api-gateway)[API Gateway] in medconnect

    service patient_app(internet)[Patient App] in clients
    service doctor_portal(internet)[Doctor Portal] in clients
    service admin_console(internet)[Admin Console] in clients

    service userpool(logos:aws-cognito)[User Pool] in cognito
    service users(database)[Users] in cognito
    service groups(database)[Groups] in cognito
    service mfa(server)[MFA] in cognito
    service app_clients(server)[App Clients] in cognito
    service triggers(logos:aws-lambda)[Lambda Triggers] in cognito

    service authorizer(logos:aws-api-gateway)[Cognito Authorizer] in api
    service endpoints(logos:aws-api-gateway)[API Endpoints] in api
...
**状況:**
医師ユーザーがMFAを設定しようとしているが、「MFA設定が利用できません」というエラーが表示される。
...
**状況:**
有効なはずのアクセストークンでAPI呼び出しをしているが、401 Unauthorizedエラーが返される。
...
**課題:**
患者向けアプリにGoogleとAppleでのソーシャルログインを追加してください。
...
**シナリオ:**
不正アクセスを検知し、リスクレベルに応じて追加認証を要求する仕組みを実装したい。
...
**次の課題**: [課題24](exercise-24.md)

**前の課題**: [課題22](exercise-22.md)
